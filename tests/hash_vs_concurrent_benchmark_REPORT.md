...
| 源码 | `tests/hash_vs_concurrent_benchmark.cc` |
| 构建 | `bazel build -c opt //tests:hash_vs_concurrent_benchmark` |
| 原始日志 | `tests/hash_vs_concurrent_benchmark_LAST_RUN.txt` |
...
```bash
cd yikv
bazel run -c opt //tests:hash_vs_concurrent_benchmark -- --benchmark_min_time=0.12s \
  | tee tests/hash_vs_concurrent_benchmark_LAST_RUN.txt
```

仅 CHM 相关：
```bash
bazel-bin/tests/hash_vs_concurrent_benchmark \
