...
- `alloc`: mmap / arena allocators
- `container`: core containers (`HashMap`, `Bitmap`, `List`, `Vector`, `String`, …); see [`container/README.md`](container/README.md) and [`container/USAGE.md`](container/USAGE.md)
- `schema`: unified schema and document metadata
- `index`: KV, inverted, and vector index interfaces and implementations

## 仓库目录布局（团队约定）

本节描述团队在 **yikv-server** 单仓中为 **库 / 常驻服务 / 批处理 Worker** 约定的 **路径语义**。**本目录 `libs/yikv`** 即 **库**（核心包 **`alloc`**、**`container`**、**`db`**、**`index`**、**`schema`** 直接置于本树下；服务端见 **`apps/yikv_server`**；离线索引与导入管线见 **`apps/workers/indexer`** 与 **`apps/workers/import_helpers`**。

### 三类构件

| 类型 | 含义 | _disk 路径示例_ |
|------|------|----------------|
| **库（libs）** | 可链接的 **`cc_library`**、KV 引擎与本目录 | **`libs/yikv/…`** |
| **常驻服务（apps）** | RPC 长驻进程 | **`apps/yikv_server`**（多于一个服务时再收 **`apps/server/<名>`**） |
| **批处理 Worker** | 导入、建索引等 Job | **`apps/workers/indexer`**、**`apps/workers/import_helpers`** |

- **indexer**、**import pipeline** 归为 **Worker**。
- **多服务**：多于一个常驻服务端二进制时，**MUST** 使用 **`apps/server/<名称>`** 收纳，勿在 **`apps/`** 根散落多个服务根。
- 公共头 **`#include "db/db.h"`** 等与 **`YIKV_COMPAT_COPTS`**（**`-Ilibs/yikv`**）一致。

以上为 **仓库级约定**；Bazel 推荐使用 **`//libs/yikv:yikv_core`** 或 **`//libs/yikv/...`**。
...
Install layout follows GNU conventions: **`$(DESTDIR)$(PREFIX)`** (default **`PREFIX=/usr/local`**). **`make install`** copies **`libyikv.so`** and **`libyikv.a`** by default (**`INSTALL_STATIC=0`** → shared only; **`INSTALL_SHARED=0`** → static archive only), **`include/yikv/`** mirrors **`{alloc,container,db,index,schema}/**/*.h`** (e.g. **`include/yikv/db/db.h`**), **`yikv-db_tool`**, README under **`share/doc/yikv`** (skip with **`INSTALL_DOCS_CLI=0`**; skip headers with **`INSTALL_HEADERS=0`**).
...
Consume installed headers as **`#include "db/db.h"`** with **`-I$PREFIX/include/yikv`** and **`-std=c++17 -pthread`**:
...
## Notes
- Prefer extending via the storage abstraction in `storage/store.h` (upstream layout) and existing Bazel targets.
- Allocators: [`alloc/README.md`](alloc/README.md). HashMap / Bitmap usage: [`container/USAGE.md`](container/USAGE.md).
...
Compatibility rules for evolving schemas are documented in `schema/schema.h` (e.g. SparseRowBinary allows appending new fields with new `field_id` values).

---

### `KVIndex` usage

Header: `index/kv_index.h`.
...
Header: `index/inverted_index.h`. Inherits all KV operations; **`Put` / `Delete`** also maintain inverted postings for indexed fields.
