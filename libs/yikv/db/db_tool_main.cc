...
// and optional read / write / SWMR benchmarks (see db/db.md).
//
// Example:
//   sudo mkdir -p /data/yikv && sudo chown "$USER" /data/yikv
//   bazel run //libs/yikv/db:db_tool -- fill --db /data/yikv --index kv_all --target-gb 6 --recreate
//   bazel run //libs/yikv/db:db_tool -- bench-read --db /data/yikv --index kv_all --ops 200000
//   bazel run //libs/yikv/db:db_tool -- bench-write --db /data/yikv --index kv_all --ops 50000
//   bazel run //libs/yikv/db:db_tool -- bench-swmr --db /data/yikv --index kv_all --threads 8 --seconds 10
