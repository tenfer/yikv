...
struct FtAllocator::Impl {
    ...
    std::uint64_t alloc_pages(std::uint64_t count) {
        ...
        if (!count || count > meta->free_page_count)
            throw std::bad_alloc();
        ...
        if (count <= 64) {
            ...
            // Rare: contiguous run crosses a 64-bit bitmap word boundary (e.g.
            // malloc_large rounded to exactly 65 pages). Scan page-by-page —
            // only used for modest counts outside the single-word fast path.
            const std::uint64_t start_p = cur % meta->page_count;
            for (std::uint64_t t = 0; t < meta->page_count; ++t) {
                const std::uint64_t p = (start_p + t) % meta->page_count;
                if (p + count > meta->page_count) break;

                bool all_free = true;
                for (std::uint64_t i = 0; i < count; ++i) {
                    const std::uint64_t pg = p + i;
                    if (bitmap_test(pg)) {
                        all_free = false;
                        break;
                    }
                ...
                meta->free_page_count -= count;
                meta->next_page_cursor = (p + count) % meta->page_count;
                const std::uint64_t off = meta->data_offset + p * meta->page_size;
                mapping.EnsureMapped(off, count * meta->page_size);
                return off;
            }
        }

        // General path: word-level bitmap scan for count > 64 or cross-word spans.
        // Uses __builtin_ctzll to jump to free runs instead of page-by-page scanning.
        // Complexity: O(words) vs O(page_count * count) for the old approach.
        {
            const std::uint64_t start_bit = cur % meta->page_count;
            const std::uint64_t start_w   = start_bit / 64;
            const std::uint64_t start_off = start_bit % 64;

            for (std::uint64_t wi = 0; wi < words; ++wi) {
                const std::uint64_t w_idx = (start_w + wi) % words;
                std::uint64_t word = bm[w_idx];

                // Skip fully-used words immediately.
                if (word == ~0ULL) continue;

                // Free bits in this word.
                std::uint64_t free = ~word;

                // Mask out bits before start offset on the first word.
                if (wi == 0 && start_off > 0)
                    free &= (~0ULL << start_off);

                // Clamp to valid pages in the last word.
                if (w_idx * 64 >= meta->page_count) continue;
                if ((w_idx + 1) * 64 > meta->page_count) {
                    std::uint64_t valid = meta->page_count - w_idx * 64;
                    free &= (1ULL << valid) - 1;
                }
                ...
                int first = __builtin_ctzll(free);
                std::uint64_t run_start = w_idx * 64 + first;
                std::uint64_t run_len   = 0;

                // Count consecutive free bits from 'first', spanning words.
                std::uint64_t cw      = w_idx;
                std::uint64_t cw_free = free;
                std::uint64_t cw_pos  = static_cast<std::uint64_t>(first);

                while (run_len < count) {
                    // How many consecutive free bits from cw_pos in current word?
                    std::uint64_t shifted = cw_free >> cw_pos;
                    if (shifted == 0) break;  // no more free bits in this word

                    // shifted has bit 0 set (since cw_pos was a free bit).
                    // Count consecutive 1s from LSB: __builtin_ctzll(~shifted).
                    std::uint64_t in_word;
                    if (~shifted == 0) {
                        // All bits from cw_pos onward are free.
                        in_word = 64 - cw_pos;
                    } else {
                        in_word = static_cast<std::uint64_t>(__builtin_ctzll(~shifted));
                    }

                    // Clamp to valid pages in the last word.
                    if ((cw + 1) * 64 > meta->page_count)
                        in_word = std::min(in_word, meta->page_count - cw * 64 - cw_pos);

                    run_len += in_word;
                    if (run_len >= count) break;

                    // Move to next word in linear page order (arena does not wrap).
                    ++cw;
                    if (cw >= words || cw * 64 >= meta->page_count)
                        break;

                    cw_free = ~bm[cw];
                    std::uint64_t valid = 64;
                    if ((cw + 1) * 64 > meta->page_count)
                        valid = meta->page_count - cw * 64;
                    cw_free &= (1ULL << valid) - 1;
                    if (cw_free == 0) break;
                    cw_pos = 0;
                }

                if (run_len >= count) {
                    // Found it — mark 'count' pages starting at run_start.
                    std::uint64_t remaining = count;
                    std::uint64_t r_pos = run_start % 64;
                    std::uint64_t r_w   = run_start / 64;
                    while (remaining > 0) {
                        std::uint64_t n = std::min(remaining, 64 - r_pos);
                        if ((r_w + 1) * 64 > meta->page_count)
                            n = std::min(n, meta->page_count - r_w * 64 - r_pos);
                    ...
                    meta->free_page_count -= count;
                    meta->next_page_cursor = (run_start + count) % meta->page_count;
                    const std::uint64_t off = meta->data_offset + run_start * meta->page_size;
                    mapping.EnsureMapped(off, count * meta->page_size);
                    return off;
                }
            }
        }
        // Last resort: exhaustive scan (correct when the word-oriented search
        // misses a contiguous run — e.g. multiple free intervals in one word).
        {
            const std::uint64_t start_p = cur % meta->page_count;
            for (std::uint64_t t = 0; t < meta->page_count; ++t) {
                const std::uint64_t p = (start_p + t) % meta->page_count;
